...
{
    /// <summary>
    /// A class representing a < see cref="SkillBase"/>  thats subitems are executet in parallel
    /// </summary>
    class Process : SkillBase
    {
        public Process(string name, IResource executer, List<ISkill> skills = null) : base(name, executer, false)
        {
            ...
            if (executingTask == null || executingTask.IsCompleted)
            {
                ...
                Task t = new Task(() =>
                {
                    ...
                    if (CanExecute())
                    {
                        ...
                        List<Task> ExecutingSkills = new List<Task>();
                        foreach (var skill in SubSkills)
                        {
                            ExecutingSkills.Add(skill.Value.Execute());                            
                        }
                        Task.WaitAll(ExecutingSkills.ToArray());
                        SetFinished();
